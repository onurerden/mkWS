 ...
 * @author onurerden
 */
@WebServlet(name = "DownloadGPX", urlPatterns = {"/DownloadGPX"})
public class DownloadGPX extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
                ...
                try (PrintWriter out = response.getWriter()) {
                System.out.println("No routeId Submited");
                response.setContentType("text/json");
                response.setStatus(401);
                out.println("{\"result\": \"failed\", \"description\" : \"No RouteId submitted.\"}");
                return;
                }
            }
        
        response.setContentType("text/gpx");
        response.setHeader("Content-Disposition", "attachment; filename=\"route_"+request.getParameter("routeId")+".gpx\"");
        ...
        try {
            ...
            for (FollowMeDataModel fm : model.getFollowMeData()) {
                ...
                outputResult = "<trkpt lon=\"" + fm.getLng() + "\" lat=\"" + fm.getLat() + "\">\n"
